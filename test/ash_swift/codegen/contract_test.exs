defmodule AshSwift.Codegen.ContractTest do
  ...
  describe "build/1 and encode/1 against the real fixture domain" do
    ...
    test "matches the committed snapshot fixture" do
      ...
      #
      # `ash_swift_version` is neutralised on both sides before comparing: it's
      # excluded metadata (moduledoc, "Some fields carry deliberately different
      ...
      # is lost). Only that one line is stripped, and from the raw text on both
      # sides — everything else about the committed artifact (pretty-printing,
      # key order, the trailing newline `--output` writes) stays asserted
      # byte-for-byte, so a fixture regenerated by any route other than the
      # documented command still fails here.
      strip_version = &String.replace(&1, ~r/^  "ash_swift_version": "[^"]*",\n/m, "")

      assert strip_version.(Contract.encode(@domains) <> "\n") ==
               strip_version.(File.read!(@fixture_path))
    end
  end
